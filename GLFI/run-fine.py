...
stable = np.array([0.96,0.94,0.93,0.92,0.91,0.88,0.85,0.81])

for j,mass in enumerate(m1):
    count = 0
    seps = np.arange(stable[j]-0.01, stable[j]+0.01,0.001)
    seps = seps[::-1]
    for i,sep in enumerate(seps):
        print('Mass =',np.log10(m1[j]),':',j+1,' of ',len(m1)+1,': Testing ',i+1,' of ',len(seps)+1, 'sep = ',np.round(sep, decimals=3))
        ...
        system = System(m1[j], pos, vel, post, velt, dt = dt, Norbits = Norbits, 
                        run_number = '-m'+str(np.abs(np.log10(m1[j])))+'-sep'+str(np.round(sep, decimals=3)))
        system.Run()
        ...
        if count == 3:
            print('3 stable orbits in a row, moving on')
            break
